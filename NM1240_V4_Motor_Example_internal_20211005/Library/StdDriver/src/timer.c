 ...
 * @brief    NM1240 TIMER driver source file
 *
 * @note
...
/**
  ...
  *                 - \ref TIMER_TRIGGER_COUNTING_CAPTURE_MODE
...
uint32_t TIMER_GetModuleClock(TIMER_T *timer)
{
    ...
    if(u32Src == 0) {
        if((CLK->PWRCTL & CLK_PWRCTL_EXTCLKEN_Msk) == 0x03 )
            return __EXT;
    }
    else if(u32Src == 1) {
        return __LIRC;
