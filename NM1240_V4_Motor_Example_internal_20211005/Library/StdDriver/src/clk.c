 ...
 * @brief    NM1240 CLK driver source file
 *
 * @note
...
/**
  * @brief      This function enable external EXT_CLK.
  * @param      None
  * @return     None
  */
void CLK_EnableEXTCLK(void)
{
    /* Enable EXT clock source */
    CLK->PWRCTL |= CLK_PWRCTL_EXT_EN;
}

/**
  * @brief      This function disable external EXT_CLK.
  * @param      None
  * @return     None
  */
void CLK_DisableEXTCLK(void)
{
    /* Disable EXT clock source */
    CLK->PWRCTL &= ~CLK_PWRCTL_EXT_EN;
}
...
/**
  * @brief      This function get external clock frequency (EXT). The frequency unit is Hz.
  * @param      None
  * @return     EXT frequency
  */
uint32_t CLK_GetEXTFreq(void)
{
    if ( ((CLK->PWRCTL & CLK_PWRCTL_EXTCLKEN_Msk) >> CLK_PWRCTL_EXTCLKEN_Pos) == 0x03 )
        return __EXT;
    else
        return 0;
}
...
uint32_t CLK_SetCoreClock(uint32_t u32Hclk)
{
    ...
    if ((u32Hclk <= FREQ_48MHZ) && (u32Hclk > 0))
    {
        CLK->PWRCTL &= ~(CLK_PWRCTL_HIRC_SEL_Msk);
        u32Div = FREQ_48MHZ / u32Hclk;
        if (FREQ_48MHZ % u32Hclk != 0)
            u32Div++;
        if (u32Div > 16)
        {
...
/**
  ...
  * |\ref TMR2_MODULE       |\ref CLK_TMR2_SRC_EXT              | x                         |
  * |\ref TMR2_MODULE       |\ref CLK_TMR2_SRC_LIRC             | x                         |
  * |\ref TMR2_MODULE       |\ref CLK_TMR2_SRC_HCLK             | x                         |
  * |\ref TMR2_MODULE       |\ref CLK_TMR2_SRC_T0               | x                         |
  * |\ref TMR2_MODULE       |\ref CLK_TMR2_SRC_HIRC             | x                         |
...
///**
//  * @brief      This function trim HIRC clock
//  * @return     None
//  * @details    To wait for HIRC calibration is finished (~10 ms)
//  */
//void CLK_HIRC_Calibration(void)
//{    
//    if (SystemCoreClock == FREQ_60MHZ) 
//    {
//        CLK->CLKDIV = 0x01;     // slow down HCLK to prevent HCLK freq change cause system error

//        SYS->IRCTCTL |= (1 << SYS_IRCTCTL_FREQSEL_Pos);
//        while(SYS->IRCTCTL & SYS_IRCTCTL_FREQSEL_Msk) ;     // wait auto trim clear by hardware
//    
//        CLK->CLKDIV = 0x00;
//    }
//    else
